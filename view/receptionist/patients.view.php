...
<?php show_msg(); ?>
<form method="get"><input type="text" name="keyword" placeholder="Name, phone or ID" value="<?php echo $keyword; ?>"><input type="submit" value="Search"></form>
<table><tr><th>ID</th><th>Name</th><th>Phone</th><th>Email</th><th>Status</th></tr>
<?php foreach($patients as $p){ ?><tr><td><?php echo $p['id']; ?></td><td><?php echo $p['name']; ?></td><td><?php echo $p['phone']; ?></td><td><?php echo $p['email']; ?></td><td><?php echo $p['is_active']; ?></td></tr><?php } ?>
</table>
...
<select name="gender"><option value="Male">Male</option><option value="Female">Female</option><option value="Other">Other</option></select><br>
